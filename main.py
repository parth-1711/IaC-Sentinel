...
from scanner.storage import log_scan, seed_demo_history
...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan_parser.add_argument("--log-db", default="iac_sentinel.db", help="Path to SQLite audit log database")
    scan_parser.add_argument("--export-dashboard-json", default="dashboard/public/violations_log.json", help="Path to dashboard JSON export")
    scan_parser.add_argument("--repo", default="terraform-production-infra", help="Repository identifier for audit logs")
    ...
    seed_parser = subparsers.add_parser("seed-dashboard", help="Seed demo history into SQLite and dashboard JSON")
    seed_parser.add_argument("--db", default="iac_sentinel.db", help="SQLite database path")
    seed_parser.add_argument("--json-out", default="dashboard/public/violations_log.json", help="Dashboard JSON path")
...
def handle_scan(args: argparse.Namespace) -> int:
    ...
    # Log to SQLite and export to dashboard JSON
    if args.log_db:
        scan_id = log_scan(
        ...
        print(f"[+] Persisted scan #{scan_id} to {args.log_db}")
...
def handle_seed_dashboard(args: argparse.Namespace) -> int:
    print(f"[*] Seeding demo compliance scan history into {args.db} and {args.json_out}...")
    seed_demo_history(db_path=args.db, export_json_path=args.json_out)
